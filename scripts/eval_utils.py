...
def get_crystals_list(
    ...
    for batch_idx, num_atom in enumerate(num_atoms.tolist()):
        ...
        #cur_prob_a = types_prob.narrow(0, start_idx, num_atom).sum(dim=0)
        cur_prob_x = frac_coords_prob.narrow(0, start_idx, num_atom).sum(dim=0)
        cur_lengths = lengths[batch_idx]
        cur_angles = angles[batch_idx]
        # cur_prob_a = types_prob[batch_idx]
        # cur_prob_x = frac_coords_prob[batch_idx]
        cur_prob_l = lattices_prob[batch_idx]
        ...
        # prob_list_a.append(cur_prob_a.cpu().numpy())
        prob_list_x.append(cur_prob_x.cpu().numpy())
        prob_list_l.append(cur_prob_l.cpu().numpy())
        start_idx = start_idx + num_atom
    # prob_list.append(prob_list_a, prob_list_x, prob_list_l)
    return crystal_array_list, [prob_list_x, prob_list_l]
...
def prop_model_eval(eval_model_name, crystal_array_list):
    ...
    return all_preds.tolist(), E_hull

...
def diffusion(loader, model, num_evals = 1, step_lr = 1e-5):
    ...
    print("num_atoms shape:", num_atoms.shape)  # num_atoms shape: torch.Size([1, 1280])
    print("atom_types shape:", atom_types.shape)  # atom_types shape: torch.Size([1, 13251])
    print("lattices shape:", lattices.shape)  # lattices shape: torch.Size([1, 1280, 3, 3])
    print("frac_coords_prob shape:", frac_coords_prob.shape)  # frac_coords_prob shape: torch.Size([1, 13251, 3])
    # print("atom_types_prob shape:", atom_types_prob.shape)
    print("lattices_prob shape:", lattices_prob.shape)  # lattices_prob shape: torch.Size([1, 1280, 3, 3])
...
def generating_data(i, loader, diff_model, model_path):
    ...
    if args.label == '':
        diff_out_name = f'SUN_diff_{i}.pt'
    else:
        diff_out_name = f'SUN_diff_{args.label}.pt'
...
def process_cif_files():
    ...
    output_folder_ehull = "cifs_ehull_0"
    output_folder_ehull_01 = "cifs_ehull_0.1"

    # 创建输出文件夹
    os.makedirs(output_folder_ehull, exist_ok=True)
    os.makedirs(output_folder_ehull_01, exist_ok=True)
    ...
    for csv_file in ["train.csv", "val.csv", "test.csv"]:
        ...
        for _, row in df.iterrows():
            formula = row["pretty_formula"]  # 假设列名是 "pretty_formula"
    ...
    # 3. 遍历 CIF 文件并统计
    count_ehull_0 = 0  # prop ≤ convex_hull_energy
    count_ehull_01 = 0  # prop ≤ convex_hull_energy + 0.1

    for cif_file in cif_files:
        # 解析化学式和形成能 (prop)
        parts = cif_file[:-4].split("_")  # 去掉 .cif 后缀再分割
        prop_str = parts[-1]  # 形成能是最后一个部分
        formula = "_".join(parts[:-1])  # 化学式是前面的部分

        try:
            prop = float(prop_str)  # CIF 文件名中的形成能
        except ValueError:
        ...
        if formula in energy_data:
            ...
            if prop <= convex_hull_energy:
                count_ehull_0 += 1
                # 复制到 cifs_ehull_0 文件夹
                src = os.path.join(cif_folder, cif_file)
                dst = os.path.join(output_folder_ehull, cif_file)
                shutil.copy2(src, dst)

            if prop <= convex_hull_energy + 0.1:
                count_ehull_01 += 1
                # 复制到 cifs_ehull_0.1 文件夹
                src = os.path.join(cif_folder, cif_file)
                dst = os.path.join(output_folder_ehull_01, cif_file)
                shutil.copy2(src, dst)
    ...
    return {
        ...
        "ehull_0": count_ehull_0,
        "ehull_01": count_ehull_01
    }
...
def main(args):
    ...
    for i in range(1, args.train_step+1):
        ...
        atom_type_stats = {}
        # 1,2 生成数据
        generating_data(i, loader, diff_model, model_path)
        # # 3. 读取数据
        ...
        # 获取优化任务的文件路径
        opt_file_path = '/home/huangjiao/csp-rl/diffcsp/prop_models/mp20/SUN_diff_'+str(i)+'.pt'
        # 获取晶体数组列表
        crys_array_list, _, [prob_x, prob_l] = get_crystal_array_list(opt_file_path, -3)
        print('Getting crystal list Done.')
        # crys_array_list是一个list，每一个元素是存储一个晶体结构A,X,L的字典。
        ...
        valid_props_expanded = props_expanded[valid_indices, :] + 6.0

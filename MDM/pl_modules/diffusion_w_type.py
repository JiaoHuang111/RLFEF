...
class CSPDiffusion(BaseModule):
    ...
    def sample(self, batch, diff_ratio = 1.0, step_lr = 1e-5):
        ...
        for t in tqdm(range(self.beta_scheduler.timesteps, 0, -1)):
            ...
            mean_l = c0 * (l_t_minus_05 - c1 * pred_l)
            mean_std_l = torch.nn.functional.softplus(sigmas * rand_l)
            pi_l = Normal(mean_l, mean_std_l)
            mean_t = c0 * (t_t_minus_05 - c1 * pred_t)
            mean_std_t = torch.nn.functional.softplus(sigmas * rand_t)
            pi_t = Normal(mean_t, mean_std_t)
            pi_x = Normal(x_t_minus_05 - step_size * pred_x, torch.nn.functional.softplus(std_x * rand_x))
            # print(t)
            # print(prob_t.shape)
            # print(prob_t)
            log_prob_t = pi_t.log_prob(pred_t)
            # print(log_prob_t.shape)
            # print(log_prob_t)
            if prob_t is None:
                prob_t = torch.zeros_like(log_prob_t)
            prob_t += log_prob_t
            log_prob_x = pi_x.log_prob(pred_x)
            if prob_x is None:
                prob_x = torch.zeros_like(log_prob_x)
            prob_x += log_prob_x
            # pred_l_view = pred_l.view(-1, 9)
            log_prob_l = pi_l.log_prob(pred_l)
            if prob_l is None:
                prob_l = torch.zeros_like(log_prob_l)
            prob_l += log_prob_l
            pred_x = pred_x * torch.sqrt(sigma_norm)
            ...
            traj[t - 1] = {
                ...
                'atom_types_prob': prob_t,
                'frac_coords_prob': prob_x,
                'lattices_prob': prob_l
            }

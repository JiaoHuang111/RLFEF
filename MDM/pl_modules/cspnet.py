...
class CSPNet(nn.Module):
    ...
    def __init__(
        ...
        # self.coord_out_mean = nn.Linear(hidden_dim, 3, bias=False)
        # self.coord_out_mean = nn.Linear(hidden_dim, 3, bias=False)
        self.coord_out = nn.Linear(hidden_dim, 3, bias=False)

        # self.dist_coord = Normal(self.coord_out_mean, self.coord_out_std)
        # self.coord_out = self.dist_coord.rsample()

        # self.lattice_out_mean = nn.Linear(hidden_dim, 9, bias=False)
        # self.lattice_out_std = nn.Linear(hidden_dim, 9, bias=False)
        self.lattice_out = nn.Linear(hidden_dim, 9, bias=False)

        # self.dist_lattice_out = Normal(self.lattice_out_mean, self.lattice_out_std)
        # self.coord_out = self.dist_lattice_out.rsample()
        # self.lattice_out = nn.Linear(hidden_dim, 9, bias = False)
        self.cutoff = cutoff
        ...
        if self.pred_type:
            # self.type_out_mean = nn.Linear(hidden_dim, MAX_ATOMIC_NUM)
            self.type_out = nn.Linear(hidden_dim, MAX_ATOMIC_NUM)

            # self.dist_type_out = Normal(self.type_out_mean, self.type_out_std)
            # self.type_out = self.dist_type_out.rsample()
            #self.type_out = nn.Linear(hidden_dim, MAX_ATOMIC_NUM)
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        # coord_out_std = self.coord_out_std(node_features)
        # # coord_out_mean = 2 * torch.tanh(coord_out_mean_tmp)
        # # coord_out_std = nn.functional.softplus(coord_out_std_tmp)
        # dist_coord = Normal(coord_out_mean, coord_out_std)
        # coord_out = dist_coord.rsample()
        ...
        lattice_out = self.lattice_out(graph_features)
        # lattice_out_std = torch.exp(self.lattice_out_std(graph_features))
        # # lattice_out_std = nn.functional.softplus(lattice_out_std_tmp)
        # dist_lattice_out = Normal(lattice_out_mean, lattice_out_std)
        # lattice_out = dist_lattice_out.rsample()
        lattice_out = lattice_out.view(-1, 3, 3)
        if self.ip:
            lattice_out = torch.einsum('bij,bjk->bik', lattice_out, lattices)
        if self.pred_type:
            type_out = self.type_out(node_features)
            # type_out_std = torch.exp(self.type_out_std(node_features))
            # dist_type_out = Normal(type_out_mean, type_out_std)
            # type_out = dist_type_out.rsample()
